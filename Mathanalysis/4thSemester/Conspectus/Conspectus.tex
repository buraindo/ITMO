...
Кроме того, пусть якобиан $\begin{bmatrix} u_{u^{'}} & u_{v^{'}} \par v_{u^{'}} & v_{v^{'}} \end{bmatrix}$ не равен 0 (то есть, существует обратная функция).\par
...
$$ = (\overrightarrow{r}_u \times \overrightarrow{r}_v) (\frac{\partial u}{\partial u^{'}} \frac{\partial v}{\partial v^{'}}-\frac{\partial v}{\partial u^{'}} \frac{\partial u}{\partial v^{'}}) (\text{поменяли знак}) = (\overrightarrow{r}_u \times \overrightarrow{r}_v) \begin{bmatrix} u_{u^{'}} & u_{v^{'}} \par v_{u^{'}} & v_{v^{'}} \end{bmatrix}$$
...
А $\overrightarrow{r}_x \times \overrightarrow{r}_y = \begin{bmatrix} i & j & k \par 1 & 0 & 0 \par 0 & 1 & 0 \end{bmatrix} = \overrightarrow{k}, \Rightarrow |\overrightarrow{r}_x \times \overrightarrow{r}_y | = 1$.\par
...
$\overrightarrow{r}_x \times \overrightarrow{r}_y = \begin{bmatrix} i & j & k \par 1 & 0 & f_x \par 0 & 1 & f_y \end{bmatrix} = i (-f_x) - j f_y + \overrightarrow{k}$.
...
$\begin{cases} x = R\cos\varphi \sin\theta \par y = R \sin \varphi \sin \theta \par z = R \cos \theta \end{cases}$.\par
Зададим поверхность:
...
$\begin{cases} x=\varrho \cos \varphi \par y = \varrho \sin \varphi \par z = \varrho^2 \end{cases}$.\par
...
У нас есть поверхностный интеграл $\iint_\Sigma (\overrightarrow{F}, \overrightarrow{n}_0) \ d\sigma$, где $\overrightarrow{F} = (P(x,y,z),Q(x,y,z),R(x,y,z))$.\par
...
Посчитаем его:
$$\begin{bmatrix} R & Q & R \par x_u & y_u & z_u \par x_v & y_v & z_v \end{bmatrix} \ dudv = (P \frac{\partial (y,z)}{\partial(u,v)} + Q \frac{\partial (z,x)}{\partial(u,v)}\text{(поменяли знак)} + R\frac{\partial (x,y)}{\partial(u,v)}) \ dudv$$
...
$$l :\begin{cases} x^2 + y^2 = a^2 \par z = a \end{cases}$$
...
$$\begin{cases} z = a \par x = a \cos t \par y = a \sin t \end{cases}$$
...
То есть, $\int_0^{\infty} = \begin{cases} 0, y = 0 \par 1, y \neq 0 \end{cases}$.\par
...
Так как $g$ равномерно сходится, то $\begin{cases} |g(c_1, y)| < \frac{\varepsilon}{2M} \par |g(c_2, y)| < \frac{\varepsilon}{2M} \end{cases}$.\par
