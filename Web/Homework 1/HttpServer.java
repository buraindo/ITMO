...
class HttpServer {
    ...
    private String calcMD5(String st) {
    ...
    private void process(Request request, Response response) throws IOException {
        ...
        //LABEL1
        File file = new File(root, java.net.URLDecoder.decode(request.getUri(), "utf8"));
        if (file.isDirectory()) {
            file = new File(root, java.net.URLDecoder.decode(request.getUri() + "/index.html", "utf8"));
        }
        if (file.isFile()) {
            byte[] body = readFile(file);
            String md5 = calcMD5(new String(body));
            //LABEL4
            if (request.getHeader("If-None-Match") != null && request.getHeader("If-None-Match").equals(md5)) {
