...
public class FrontServlet extends HttpServlet {
    ...
    private void processRoute(Route route, HttpServletRequest request, HttpServletResponse response) throws NotFoundException, ServletException, IOException {
        ...
        for (Class<?> c = clazz; method == null && c != null; c = c.getSuperclass()) {
            List<Method> methodList = Arrays.stream(c.getDeclaredMethods()).filter((Method m) -> m.getName().equals(route.getAction())).collect(Collectors.toList());
        ...
        for (Class<?> c = clazz; beforeMethod == null && c != null; c = c.getSuperclass()) {
            List<Method> methodList = Arrays.stream(c.getDeclaredMethods()).filter((Method m) -> m.getName().equals("before")).collect(Collectors.toList());
            if (methodList.size() > 0) {
        ...
        for (Class<?> c = clazz; afterMethod == null && c != null; c = c.getSuperclass()) {
            List<Method> methodList = Arrays.stream(c.getDeclaredMethods()).filter((Method m) -> m.getName().equals("after")).collect(Collectors.toList());
            if (methodList.size() > 0) {
        ...
        try {
            ...
            if (params.length == 1) {
                if (params[0].getType() == Map.class) {
                    actualParams = new Object[]{view};
                } else if (params[0].getType() == HttpServletRequest.class) {
                    actualParams = new Object[]{request};
                }
            } else if (params.length == 2) {
                actualParams = new Object[]{request, view};
            }
            beforeMethod.invoke(page, request, view);
            method.invoke(page, actualParams);
