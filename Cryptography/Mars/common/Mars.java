...
public class Mars {
  ...
  public void generateSubKeys(final byte[] key) {
    final var keyLen = key.length;
    final var K = this.K;
    final var T = new int[15];
    ...
    for (var j = 0; j < 4; j++) {
      ...
      for (var i = 0; i < 15; i++) {
        T[i] ^= shiftLeft(T[(i + 8) % 15] ^ T[(i + 13) % 15], 3) ^ (4 * i + j);
      }
      for (var k = 0; k < 4; k++) {
        // S-box Based Stirring of Key-Words
        for (var i = 0; i < 15; i++) {
          T[i] = shiftLeft(T[i] + S[T[(i + 14) % 15] & 0x1FF], 9);
        }
    ...
    final var B = new int[] {0xa4a8d57b, 0x5b5d193b, 0xc8a8309b, 0x73f9a978};
    int m, p, r, w, j;
    for (var i = 5; i <= 35; i += 2) {
      j = K[i] & 0x3;
      w = K[i] | 0x3;
      m = computeMask(w);
      r = K[i - 1] & 0x1F;
  ...
  */
  private int computeMask(final int x) {
    var m = (~x ^ (x >>> 1)) & 0x7fffffff;
    m &= (m >>> 1) & (m >>> 2);
    m &= (m >>> 3) & (m >>> 6);
    m <<= 1;
    m |= (m << 1);
  ...
  private int shiftLeft(final int n, final int distance) {
    return (n << distance) | (n >>> (32 - distance));
  }

  private int[] splitInput(final byte[] block) {
    final var result = new int[4];
    var index = 0;
  ...
  private int[] E(final int number, final int firstKey, final int secondKey) {
    var M = number + firstKey;
    var R = (number << 13 | number >>> 19) * secondKey;
    var i = M & 0x1FF;
    var L = S[i];
    R = R << 5 | R >>> 27;
    var r = R & 0x1F;
