...
public class Program {
    ...
    private static void findNGrams(final String text, final int n, final String filename) throws IOException {
        ...
        var result = nGrams.entrySet().stream().sorted(Comparator.comparing(Map.Entry::getValue)).collect(Collectors.toList());
        Collections.reverse(result);
        try (final var writer = Files.newBufferedWriter(Paths.get(String.format("res/vigenere/kasiski/%s.txt", filename.toLowerCase())))) {
            writer.write(String.format("%s results: \n", filename));
            for (final var biGram : result) {
                writer.write(String.format("\t%s : %d, Distances: %s\n", biGram.getKey(), biGram.getValue(), Arrays.toString(distances.get(biGram.getKey()).toArray(Integer[]::new))));
            }
        }
        if (n == 2) {
    ...
    private static void findMostProbableLength() {
        ...
        final var length = result.entrySet().stream().max(Comparator.comparing(Map.Entry::getValue)).orElseThrow().getKey();
    ...
    private static void print(final String text, final int n, final Integer th) throws IOException {
        ...
        try (final var writer = Files.newBufferedWriter(Paths.get("res/vigenere/kasiski/split.txt"))) {
            ...
            for (var i = 0; i < split.size(); i += x) {
                ...
                if (th != null) {
                    s = s.stream().map(str -> String.valueOf(str.charAt(Math.min(th - 1, str.length() - 1)))).collect(Collectors.toList());
            ...
            if (th != null) {
                writer.write(letters.entrySet().stream().sorted((l, r) -> r.getValue().compareTo(l.getValue())).map(e -> String.format("%s: %d", e.getKey(), e.getValue())).collect(Collectors.joining("\n")));
                writer.newLine();
            }
        }
    }
    ...
    private static void analyze(final String text, final int n) {
        ...
        for (var th = 0; th < n; th++) {
            ...
            final var s = split.stream().map(str -> String.valueOf(str.charAt(Math.min(finalTh, str.length() - 1)))).collect(Collectors.toList());
            s.forEach(l -> letters.put(l, letters.containsKey(l) ? letters.get(l) + 1 : 1));
            lettersByPosition.put(th, letters);
            result.append(letters.entrySet().stream().max(Comparator.comparing(Map.Entry::getValue)).map(Map.Entry::getKey).orElseThrow());
        }
    ...
    private static void go(final int n, final int letter) {
        ...
        for (var th = 0; th < n; th++) {
            ...
            if (th == letter - 1) {
                ...
                lettersByPosition.get(th).entrySet().removeIf(e -> e.getKey().equals(sorted.get(0).getKey()));
    ...
    public static void main(final String[] args) throws IOException {
        var text = String.join(" ", Files.readAllLines(Paths.get("res/vigenere/kasiski/input.txt"))).toLowerCase().replaceAll("[^a-z]", "");
        ...
        while (true) {
            ...
            if (command.equals("/f")) {
                ...
                text = String.join(" ", Files.readAllLines(Paths.get("res/vigenere/kasiski/input.txt"))).toLowerCase().replaceAll("[^a-z]", "");
                ...
                print(text, Integer.parseInt(command.split(" ")[1]), answer.toLowerCase().equals("no") ? null : Integer.parseInt(answer));
